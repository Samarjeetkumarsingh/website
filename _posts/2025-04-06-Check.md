...
<body>
    ...
    <script>
        ...
        const chartSection = document.querySelector('.performance-milestones');
        const chartObserver = new IntersectionObserver((entries) => {
            entries.forEach(entry => {
                if (entry.isIntersecting) {
                    // Rotate chart slightly for effect
                    performanceChart.options.rotation = -0.1 * Math.PI;
                    ...
                    chartSection.addEventListener('mousemove', (e) => {
                        const x = e.clientX / window.innerWidth;
                        const y = e.clientY / window.innerHeight;
                        performanceChart.options.rotation = -0.1 * Math.PI + (x - 0.5) * 0.2;
                        performanceChart.options.circumference = 2 * Math.PI - (y * 0.2 * Math.PI);
                        performanceChart.update();
                    });
                }
            });
